...
//        }
    ...
    lazy var broadcastPicker: RPSystemBroadcastPickerView = {
        ...
        systemBroadcastPicker.preferredExtension = "com.mediadh.App.Learnalize.learnalize-screenshare"
...
//        view.layer.borderWidth = 2
    ...
    @objc func endButtonTapped() {
        if localPeer?.role?.permissions.endRoom == true {
            ...
            print("you are not allowed to end this room")
        }
    }
...
//    }
    ...
    fileprivate func setupPrivilegedGuestUI() {
        print("*******setting up Privileged guest UI*******")
//        setupLocalVideoView()
        audioButton.isHidden = false
        videoButton.isHidden = false
        broadcastPicker.isHidden = false
...
extension RoomController:UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        print("number of remote peers in the room is \(hmsSDK.remotePeers?.count ?? 0)")
...
extension RoomController: HMSUpdateListener {
    ...
    func on(error: Error) {
        print("couldnt join room because \(error.localizedDescription)")
    }
    ...
    func on(peer: HMSPeer, update: HMSPeerUpdate) {
        ...
        case .roleUpdated:
            ...
            if localPeer?.role?.name == "guest" {
                ...
                setupPrivilegedGuestUI()
                videoButton.isHidden = false
                participant?.role = .privilegedGuest
